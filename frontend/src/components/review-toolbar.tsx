...
export function ReviewToolbar({
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 bg-muted border-t border-border rounded-b-md">
      <button
        onClick={onApprove}
      ...
      {onSkip && (
        <button
          onClick={onSkip}
          disabled={isSubmitting}
          className="flex items-center gap-2 rounded-md border border-border px-4 py-2 text-sm text-muted-foreground hover:bg-muted disabled:opacity-50"
        >
          <SkipForward size={16} />
          Skip
        </button>
